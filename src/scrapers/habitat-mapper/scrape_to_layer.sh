...
function scrape_year() {
	...
	for month in {1..11}; do
		...
		python3 scrape_to_layer.py "$start" "$end"
	done

	# scrape last month
	#
	local next_year=$((year + 1))
	python3 scrape_to_layer.py "$year-12-01" "$next_year-01-01"
}
...
function scrape_past_3_months() {
	python3 scrape_to_layer.py '2015-08-01' '2015-09-01'
	python3 scrape_to_layer.py '2015-09-01' '2015-10-01'
	python3 scrape_to_layer.py '2015-10-01' '2015-11-01'
}
